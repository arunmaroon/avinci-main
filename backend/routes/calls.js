...
router.post('/process-speech', async (req, res) => {
    try {
        ...
        try {
            ...
            // For group calls, also get multiple responses
            if (type === 'group') {
                try {
                    ...
                    if (groupResponse.data && Array.isArray(groupResponse.data)) {
                        console.log(`🎭 Group call: Generated ${groupResponse.data.length} agent responses`);
                        
                        // Emit all responses simultaneously for group conversation
                        const roomName = `call-${callId}`;
                        console.log(`🎭 Group call: ${groupResponse.data.length} agents responding simultaneously`);
                        
                        // Emit all typing indicators at once
                        groupResponse.data.forEach((response, index) => {
                        ...
                        setTimeout(() => {
                            groupResponse.data.forEach((response, index) => {
                                ...
                                console.log(`✅ Agent response ${index + 1} sent simultaneously:`, response.agentName);
                            });
                        }, 1000); // All agents respond together after 1 second
                    }
                } catch (groupError) {
                    console.warn('Group overlap service unavailable:', groupError.message);
                }
            }
