...
const agentProfiles = {
    'Operations Manager': {
        ...
        goals_detail: {
            primary: ['Optimize operational efficiency', 'Implement digital transformation', 'Mentor junior staff'],
            secondary: ['Learn new technologies', 'Build industry network', 'Achieve work-life balance']
        },
        pain_points: ['Legacy systems', 'Resistance to change', 'Budget constraints', 'Staff turnover'],
        pain_points_detail: {
            primary: ['Legacy systems', 'Resistance to change', 'Budget constraints', 'Staff turnover'],
            frustrations: ['Inefficient processes', 'Poor communication', 'Lack of resources'],
            ux_issues: ['Complex interfaces', 'Slow loading times', 'Poor mobile experience']
        },
        ...
        life_events: [
            { milestone: 'Marriage', year: 2016, impact: 'High', description: 'Married college sweetheart, moved to new city' },
            { milestone: 'First Child', year: 2018, impact: 'High', description: 'Became a father, changed priorities and work-life balance' },
            { milestone: 'Promotion to Manager', year: 2021, impact: 'High', description: 'Promoted to operations manager, increased responsibilities' }
        ],
        recommendations: 'Focus on process automation, invest in team training, maintain work-life balance',
        beliefs: 'Efficiency and process improvement are key to success. Hard work, integrity, family first, community service.'
    },
    'Research Director': {
        age: 42,
        gender: 'M',
        ...
        life_events: [
            { milestone: 'PhD Completion', year: 2012, impact: 'High', description: 'Completed PhD in Market Research, started academic career' },
            { milestone: 'First Major Publication', year: 2016, impact: 'High', description: 'Published breakthrough research paper, gained recognition' },
            { milestone: 'Marriage', year: 2019, impact: 'High', description: 'Married research colleague, moved to new city together' }
        ],
        recommendations: 'Invest in advanced research tools, maintain academic connections, publish regularly',
        beliefs: 'Evidence-based decisions lead to better outcomes. Knowledge sharing, academic integrity, research ethics.'
    },
    'Data Analyst': {
        age: 28,
        gender: 'M',
        ...
        life_events: [
            { milestone: 'Graduation', year: 2021, impact: 'High', description: 'Completed B.Tech in Computer Science, started job search' },
            { milestone: 'First Job', year: 2022, impact: 'High', description: 'Joined tech company as data analyst, learned industry practices' },
            { milestone: 'City Move', year: 2023, impact: 'Medium', description: 'Moved to current city for better career opportunities' }
        ],
...
async function enrichAgents() {
    try {
        ...
        for (let i = 0; i < agents.length; i++) {
            ...
            // Create enriched profile with only existing columns
            const enrichedProfile = {
                age: baseProfile.age + Math.floor(Math.random() * 10) - 5,
                gender: baseProfile.gender,
                education: baseProfile.education,
                income_range: baseProfile.income_range,
                personality: JSON.stringify({
                    traits: baseProfile.personality.split(', '),
                    communication_style: baseProfile.communication_style,
                    decision_style: baseProfile.decision_style,
                    values: baseProfile.community_values.split(', ')
                }),
                personality_profile: baseProfile.personality.split(', '),
                background_story: baseProfile.background_story,
                goals: baseProfile.goals,
                goals_detail: JSON.stringify(baseProfile.goals_detail || { primary: baseProfile.goals, secondary: [] }),
                pain_points: baseProfile.pain_points,
                pain_points_detail: JSON.stringify(baseProfile.pain_points_detail || { primary: baseProfile.pain_points, frustrations: [], ux_issues: [] }),
        ...
        occupation: 'Operations Manager',
            ...
            // Update the agent with only existing columns
            await pool.query(`
                UPDATE ai_agents SET 
                    ...
                    english_level = $30,
                    tech_savviness = $31,
                    financial_savviness = $32,
                    product_familiarity = $33,
                    employment_type = $34,
                    vocabulary_profile = $35,
                    cognitive_profile = $36,
                    knowledge_bounds = $37,
                    extrapolation_rules = $38,
                    source_meta = $39,
                    background = $40,
                    quote = $41,
                    master_system_prompt = $42,
                    domain_literacy = $43,
                    cognitive_characteristics = $44,
                    knowledge_areas = $45,
                    ui_pain_points = $46,
